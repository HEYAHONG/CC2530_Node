...
#define HAL_DMA_SET_ADDR_DESC0( a ) \
  st( \
    ...
    DMA0CFGL = (uint8_t)( (uint16_t)(a) & 0xFF );       \
  )

#define HAL_DMA_SET_ADDR_DESC1234( a ) \
  st( \
    DMA1CFGH = (uint8_t)( (uint16_t)(a) >> 8 );  \
    DMA1CFGL = (uint8_t)( (uint16_t)(a) & 0xFF );       \
  )
...
#define HAL_DMA_SET_SOURCE( pDesc, src ) \
  st( \
    pDesc->srcAddrH = (uint8_t)( (uint16_t)(src) >> 8 ); \
    pDesc->srcAddrL = (uint8_t)( (uint16_t)(src) & 0xFF ); \
  )

// Macro for quickly setting the destination address of a DMA structure.
#define HAL_DMA_SET_DEST( pDesc, dst ) \
  st( \
    pDesc->dstAddrH = (uint8_t)( (uint16_t)(dst) >> 8 ); \
    pDesc->dstAddrL = (uint8_t)( (uint16_t)(dst) & 0xFF ); \
  )
...
#define HAL_DMA_SET_LEN( pDesc, len ) \
  st( \
    pDesc->xferLenL = (uint8_t)( (uint16_t)(len) & 0xFF); \
    pDesc->xferLenV &= ~HAL_DMA_LEN_H; \
    pDesc->xferLenV |= (uint8_t)((uint16_t)(len) >> 8); \
  )

#define HAL_DMA_GET_LEN( pDesc ) \
  (((uint16_t)(pDesc->xferLenV & HAL_DMA_LEN_H) << 8) | pDesc->xferLenL)
